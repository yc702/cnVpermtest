#' A CNA permutation function for deletion only
#'
#' This function allows you to do permutation of CNA segments for deletion only.
...
#' @param permtime Permutation time
...
## Only consider deletion

tcgacnv_del<-function(cnvdatalist,segmentmean_name,segment_cutoff,permtime,core){
  ...
  len.p <- len.p[sapply(len.p, function(x) x!=0)]
  range.p <- range.p[sapply(range.p, function(x) !is.logical(x))]
  size.p <- unname(size.p[sapply(size.p, function(x) !is.logical(x))])
  chr.name.p <- unname(chr.name.p[sapply(chr.name.p, function(x) length(x)!=0)])
